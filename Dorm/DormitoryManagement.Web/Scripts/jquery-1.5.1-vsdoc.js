/*
 * This BaseFile has been commented to support Visual Studio Intellisense.
 * You should not use this BaseFile at runtime inside the browser--it is only
...
/*!
 * Note: While Microsoft is not the author of this BaseFile, Microsoft is
 * offering you a license subject to the terms of the Microsoft Software
 * License Terms for Microsoft ASP.NET Model View Controller 3.
 * Microsoft reserves all other rights. The notices below are provided
 * for informational purposes only and are not the license terms under
 * which Microsoft distributed this BaseFile.
 *
...
jQuery.fn.extend({
	load: function( url, params, callback ) {
		///	<summary>
		///     &#10;Loads HTML from a remote BaseFile and injects it into the DOM.  By default performs a GET request, but if parameters are included
...
jQuery.extend({
	...
	getScript: function( url, callback ) {
		///	<summary>
		///     &#10;Loads and executes a local JavaScript BaseFile using an HTTP GET request.
		///	</summary>
...
jQuery.extend({
	...
	httpSuccess: function( xhr ) {
		...
		try {
			...
			return !xhr.status && location.protocol === "BaseFile:" ||
...
if ( window.ActiveXObject ) {
	jQuery.ajaxSettings.xhr = function() {
		if ( window.location.protocol !== "BaseFile:" ) {
			try {
				return new window.XMLHttpRequest();
			} catch(xhrError) {}
		}
